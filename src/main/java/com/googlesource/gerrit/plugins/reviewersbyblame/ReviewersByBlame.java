...
public class ReviewersByBlame implements Runnable {
  ...
  @Inject
  public ReviewersByBlame(@Assisted final RevCommit commit,
      @Assisted final Change change, @Assisted final PatchSet ps,
      @Assisted final int maxReviewers, final ProjectCache projectCache,
  ...
  public void run() {
    ...
    PatchList patchList;
    if (commit.getParentCount() == 1 // Ignore merges and initial commit.
        && (patchList = getPatchList(change, ps)) != null) {
      for (PatchListEntry entry : patchList.getPatches()) {
        BlameResult blameResult;
        if ((entry.getChangeType() == ChangeType.MODIFIED || entry
            .getChangeType() == ChangeType.DELETED)
      ...
      Set<Account.Id> topReviewers =
          findTopReviewers(Collections.unmodifiableSet(reviewers.entrySet()),
              maxReviewers);
  ...
  /**
   * Get a list of {@link PatchListEntry}, this method is useful to get a list
   * of all changed files in this specific {@link Change}
   *
   * @param change Change to get {@link PatchListEntry} from
   * @param ps {@link PatchSet}
   * @return a {@link List} of {@link PatchListEntry}, null if it's not
   *         available
   */
  private PatchList getPatchList(final Change change, final PatchSet ps) {
    try {
      return patchListCache.get(change, ps);
